...
var avg_font_size = Math.ceil(apply_on_text_elements(averager));
...
for (var i = 1; headings[i]!=null; i++) {  
  if (headings[i].size < headings[i-1].size && headings[i-1].level < 5) {       // smaller than previous
  ...
  write_to_file("\nAssigning " + headings[i].text.contents + " to level " + headings[i].level);
...
for (var i=0;i<6;i++) { //apply_on_text_elements over the rows in headings_for_level
  for (var j=0; j<headings_for_level[i].length;j++) { //apply_on_text_elements over alll elemets of one row
    //apply the custom paragraph style to the paragraph  
    headings_for_level[i][j].text.appliedParagraphStyle = app.documents[0].paragraphStyles.item (headings_for_level[i][j].style.name); 
...
function averager() {
  ...
  return {
    fun: function(myText) {
            sum = sum+myText.pointSize*myText.contents.length;
            count = count+myText.contents.length;
         },
    val: function() {
            return sum/count;
         }
...
function style_creator() {
  ...
  function create_style(myText) {
    ...
    for (var i = paraStyles.length-1; i > 0; i--) {
      if (paraStyles[i].appliedFont == myText.appliedFont      
        && paraStyles[i].fontStyle == myText.fontStyle 
        && paraStyles[i].pointSize == myText.pointSize )
        write_to_file("\nsame as " + i);
        return paraStyles[i];
    }

    var styleName = "autogenerated_" + headings.length;       

    try {
      return app.documents[0].paragraphStyles.add({ name : styleName,
    ...
    fun: function(myText) {
            if (myText.pointSize > avg_font_size) {  // distinguish between non heading and heading text by using the average
              ...
              var s = { text : myText ,
                         ...
                         level : 0 ,
                         style : create_style(myText)};
              headings.push(s);
            } else {
              write_to_file("\nSkipped " + myText.contents + " of size " + myText.pointSize);
            }
    },
